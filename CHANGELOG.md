...
- **数字に挟まれたハイフンを 「マイナス」 と読んでいた** (「ゲームセット 3-2」 →
  「サンマイナスニ」)。 実データ 704 万行のハイフン 4,857 行を調べたところ、
...
- **単独の 「N 日」 が counter の `specials` を無視していた**。 暦の日付は
  ...
  指定できるようになり 「2 日」 が ニニチ → フツカ に (実データ 3,677 行が改善)。
...
- **`split_for_aquestalk` が 「句境界の無い長大入力」 を分割できなかった**。
  ...
  実コーパス 704 万行の構造検査 (不正文字 / 核の重複 / 句数 / 長さ) で検出。
...
- **空白の直後の数字で助数詞 / 単位が効かなかった** (「優勝 5回」 → 「ゆうしょう 5かい」)。
  ...
  (従来は句全体が 1 range で lookup miss していた)。 実コーパス 704 万行で 7,607 行が変化。
...
- **数値 + 大数スケール の直後に来る 「読めない単位」 を surface のまま読みに混ぜていた**
  (実コーパス 704 万行の走査で発覚: 読みに漢字が残る行 1,050 件の主因)。
