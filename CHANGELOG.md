...
- **IPADIC の接尾辞用の連濁読みが連濁の起きない位置でも出ていた** (お疲れ様 → おづかれ /
  ぺこらの勝ちだ → がち / 書きはムリ → がき / ラーメン食いに行こう → ぐい / トドメ刺しにきてる → ざし)。
  IPADIC は 疲れ=ヅカレ / 使い=ヅカイ / 書き=ガキ 等を 名詞-接尾 として持ち、 文頭や助詞・未知語の直後にも
  付けていた。 Lindera fallback で、 漢字 + 送り仮名の 名詞-接尾 が境界 (文頭 / 句読点・括弧・空白 /
  助詞 / 接頭詞 お・ご / ひらがなで終わる未知語) の直後か 「に + 移動動詞」 の前に来た時だけ先頭を清音に戻す。
  名詞の後ろの連濁 (魔物使い / 野菜嫌い / 一人勝ち) と、 名詞の後ろで濁るのが普通の 通り / 越し / 帰り /
  攻め / 沿い / 伝い、 元が濁音の 出し は対象外。 実コーパス 593 万行 A/B: 863 行変化 (勝ち猫 絵文字 439 /
  お疲れ 150+ / X食いに行く / 積み・書き の文頭 等)、 目視で退行 5 行 (ぬか漬け / 雰囲気作りに / ぷよ使いの)。
  処理時間の悪化なし

- **位取り漢数字 + 助数詞の先頭桁落ち** (第六九条 → きゅうじょう / 七四条 → よんじょう /
  第二五七条 → ななじょう)。 `kansuji_to_arabic` が単位 (十/百/千) の無い桁列を additive で
  解いて最後の桁だけ残していた。 単位も 〇 も無い 2 桁以上は positional (六九 = 69) に。
  連続 2 桁の概数 (二三日 / 四五人) は従来通り。 法律・判例文の条番号で頻出

- **dict entry の直後が行き止まりになる入力で entry が黙って無視されていた**
  (断捨離して → だんしゃはなして / 堂安いけ → どうやすいけ / 神木隆之介さん → しぼくたかゆきかいさん)。
  IPADIC が 「離し」 「安い」 「介さ」 のように漢字 + かなを 1 語にすると、 entry の
  終わりから始まる edge が無く、 **entry を通る path が DP 上そもそも組めない**
  (ADR-0011 が挙げた dead-end)。 Viterbi が 「dict entry (band 1000 以上) がここで終わり、
  候補が 1 つも無い」 と確認した位置でだけ、 形態素 layer に token の残り (かなだけの時)
  を補わせる `CandidateProvider::dead_end_candidates_at` を追加。 entry の読みが残りの
  先頭かなで終わる形 (天の助 = てんのすけ + け) は補わない。 数字 provider (950) は対象外
  (5回戦えば / 三段落ち が壊れる)。 実コーパス 562 万行 A/B: 94 行変化、 改善 ≈85
  (断捨離 / 堂安い / 大喜利 / 牛若丸 / 幻想水滸伝 / 厚労省 / 兎に角)、 退行 9 は entry 側の
  形の問題 (前掛 / 虎落 / 釣瓶落 / 上手+い)。 処理時間 +2.3%。

## [0.4.6] - 2026-09-19

### Added

- **`tts::silence_flags` を公開**。 `filter_tokens_for_tts` は `Vec<ReadingToken>` を
  要求するため、 **`analyze` の token から自前で読みを組み立てる caller** (= HTTP wrapper)
  が同じ判定を使えなかった。 判定本体を `silence_flags(&[&str]) -> Vec<bool>` として
  切り出して公開する (`filter_tokens_for_tts` はこれを呼ぶだけになり、 挙動は不変)。
  実際に本番の読み上げで 「かわいい(´・ω・`)」 が 「かわいい ́おめが`」 になっていた
  (0.3.0 で `silence_symbols` を足した時、 wrapper 側の配線が漏れていた)。

### Fixed

- **1 字一段動詞の語幹格上げを複合動詞にも広げた** (見逃がし → みのがし)。 直後が
  「助動詞 / 接続助詞 / 非自立動詞」 に限っていた条件を **動詞全般** (自立含む) へ拡張。
  実コーパス A/B: 対象語で 42 行改善、 退行 0。

- **かなの助数詞 (2つ / 3つ) に `match_hits` を付けた**。 漢字の助数詞に付けると
  「腹八分目」 「3時間位」 が壊れるので、 `counter.chars().all(is_hiragana_char)` の時だけ。

- **助詞 1 文字だけの tail にも edge を出す** (1つ目と → ひとつめと)。 IPADIC が
  「目と」 「円か」 を 1 語で持つため entry を通る path が組めず、 いちつもく /
  いちおくまど になっていた。 `の` は 「100万分の1」 を壊すので除く。
...
- **同じ漢字の繰り返しで先頭だけ読みが変わる問題を修正** (海海海 → うみうみうみ)。
  `[[kanji]]` block の 「前が漢字なら音読み」 型 match (海 = default うみ / 漢字の後 カイ) は、
  強調の繰り返しでは 2 文字目以降だけ前が同じ字 (= 漢字) になり 「うみかいかい」 になっていた。
  同じ字の連続は熟語の隣接ではないので、 連続全体を 1 単位として **その外側の文字** で
  文脈を判定する。 連続の後ろの送り仮名は最後の字にだけ係る (「遺言書書いて」 の 1 つ目の 書)。
  正規の重ね語 (堂堂 等) は band 1000 の dict entry が勝つので不変。
  実コーパス A/B: 同字連続を含む行で 478 行が変化 (お手手 → おてて / 車車車 / 石石 / 汗汗 /
  強強 → つよつよ 等)。

- **1 字だけ切り出された一段動詞の読みを修正** (寝んな → ねんな)。 Lindera が動詞語幹だけを
  1 字 token にすると (寝|ん / 出|ん / 見|終わ)、 活用込みの正しい読み (ネ / デ / ミ) を
  持っていても band 50 のため単漢字 default (シン / シュツ / ケン) に負けていた。
  送り仮名の形を辞書で列挙しきれないので lib 側で扱う: **動詞/自立 かつ 活用型 一段**、
  **直後が助動詞 (断定 だ/です を除く) / 接続助詞 / 非自立動詞**、 **直前が漢字・接頭の お/ご でない**
  時だけ band 100 + match_hits 1 に格上げする (送り仮名 literal に一致した `[[kanji]]` block には
  引き続き負ける)。 得 は名詞 とく との取り違えが多いため対象外。
  実コーパス A/B: 対象字を含む 30 万行で 480 行が変化 (見終わ → みおわ / 寝とる → ねとる /
  出とる → でとる / 経ない → へない 等)。

### Performance

- **解析 1 回あたりの確保を 35〜43% 削減** (出力は完全に同一)。
  - Lindera の `Lattice` を `Mutex` 内で使い回す (`tokenize_with_lattice`)。 短文 1 回で
    約 14 KiB を作り直していた最大項。 64 KiB を超える入力の後は作り直して常駐メモリを抑える。
  - pipeline 内部の形態素解析を軽量版に (品詞・活用・原形の `String` を作らない)。
  - provider → Viterbi 間を内部型 `RawCandidate` (surface なし / reading は借用) にし、
    公開型 `Candidate` へは採択 path と `analyze()` の `candidates` だけ変換。
    **公開 API は不変** (provider trait は crate 内部)。
  - `Dict::rich_matching_prefix` の先頭 2 文字を `String` に組まず入力の slice を使う。
  - `resolve_readings` を iterator 化 (候補ごとの `Vec` 確保を除去)。
  - Lindera fallback の位置 lookup を全 edge 走査から二分探索に (長い入力での O(N²) 除去)。

  実コメント行で allocs 1,588k → 1,035k (-35%) / bytes 305 MB → 187 MB (-39%)、
  長文で allocs -43%。 `to_ruby` 出力と `analyze()` JSON は変更前と完全一致。

### Added

- **コスト lattice engine** (`FuriganaBuilder::cost_engine` / `FURIGANA_COST_ENGINE=1`、
  **実験中の opt-in**、 既定は従来の band engine)。 IPADIC の語コスト + 連接コストと
  dict 候補を 1 本の lattice に並べ、 コスト最小 path を選ぶ (ADR-0011)。
  Lindera が誤分割した時に辞書でも lib でも直せない構造 (数値上げ = すうねあげ) への対処。
  現状 corpus 98.4% / 実コメント 30 万行で 43,000 行/秒 (band engine は 100% / 78,000 行/秒)。

### Documentation

- `docs/PERFORMANCE.md` に **性能目標** を明記 (下限 = 実コメント行 1 スレッド 10,000 行/秒、
  長文 1,000 文字 5 ms、 入力長に線形、 1 変更あたり実コーパス A/B で +10% 以内) と、
  criterion ではなく **両版交互 3 回の最小時間 + 出力差分全件** で判定する A/B 手順。
...
- **辞書 lookup の hot path を再度高速化 (実文で 1.4〜1.8 倍速)**。
  `DictBridgeProvider` は先頭 1 文字で bucket を引いた後 **bucket 全件を走査** して
  前方一致を試していた。 辞書の先頭文字 bucket は大半が 1 件 (p50=1 / p90=1) だが、
  実文で頻出する字にだけ集中していた (御 713 / 大 358 / 三 187 / 小 184 / 天 172 /
  一 163) ため、 「御」 が 1 文字出るたび 713 回の前方一致判定が走っていた。
  bucket は列挙順を決定的にするため surface 昇順 sort 済みなので、 **先頭 2 文字が
  一致する連続区間を二分探索で取る** ようにして実走査を数件に縮小
  (`Dict::rich_matching_prefix`)。 列挙順は従来と同一 (昇順) で、 候補集合・
  tie-break・`analyze()` の alternatives 順序はいずれも不変。

  criterion 実測 (実辞書 66,382 entry、 いずれも p < 0.05):
  short 11.5 → 8.9 µs (-22%) / short_phrase 24.5 → 14.2 µs (-43%) /
  medium 85.3 → 54.6 µs (-37%) / long 287 → 213 µs (-28%)。
  辞書が今後太っても走査量は増えない (実走査 = その 2 文字で始まる entry 数)。
...
- 百の位で終わる数 + 促音化する助数詞が促音にならなかったのを修正
  (`100本` → ヒャクポン / `100回` → ヒャクカイ / `600本` → ロッピャクポン)。
  カ行・パ行で始まる助数詞の前でだけ ヒャク / ピャク / ビャク を促音にする
  (ヒャッポン / ヒャッカイ / ロッピャッポン)。 サ行・タ行 (`100歳` / `100点`) は
  ヒャク読みが一般的なので変えない
...
- `furigana lookup --batch`: stdin を 1 行 1 入力として読み、 1 行 1 結果を出す。
  辞書 load が 1 回で済むので、 多数の入力をまとめて変換する用途 (回帰テスト等) で
  プロセス起動あたりのコストが消える。 出力が複数行になる mode (`analyze` / `accent` /
  `aquestalk --max-len`) はエラーにして行対応の崩れを防ぐ。

### Fixed

- **数字に挟まれたハイフンを 「マイナス」 と読んでいた** (「ゲームセット 3-2」 →
  「サンマイナスニ」)。 内部評価コーパスのハイフン 4,857 行を調べたところ、
  スコア (3-1 / 22-0)・面数 (マリオ 2-1)・年範囲 (2024-2025)・型番 (RX-78-2) ばかりで
  **減算の例が 1 件も無かった**。 符号と見なす条件を 「直後が数値、 かつ直前が
  数値でも英字でもない」 に厳格化 (負数は 「-3」 「弾道 -3」 の形)。
  4,857 行中 4,685 行が変化し、 マイナス読みが残るのは真の負数 22 → 14 行のみ。

- **解決できない単位で文字が黙って消えていた** (「4G」 → 「ヨン」)。 SI 単位の
  span を取る regex は case-insensitive だが、 units 側が `ci = false` の時は
  lookup が miss し、 空文字を連結していた。 解決できない span は候補ごと捨て、
  alphabet passthrough に委ねる (「4G」 → 「ヨン G」)。

- **単独の 「N 日」 が counter の `specials` を無視していた**。 暦の日付は
  `days.toml` が見るので単独形は default 直行という実装だったが、 counter 側に
  書かれた specials まで読み飛ばしていた。 dict 側で 2〜10 / 14 / 20 / 24 を
  指定できるようになり 「2 日」 が ニニチ → フツカ に (内部評価コーパスで 3,677 行が改善)。

- **`silence_symbols` が顔文字を 2 通りの形で取りこぼしていた**。 (1) 装飾に
  挟まれた 1〜2 文字の英字 (`\(^o^)/` の o、 `d(^-^)` の d) は英数字判定で
  残り 「テンクウジョウ オー」 と読まれていた。 (2) 装飾と句読点が 1 token に
  混ざる場合 (`キタ━━(゚∀゚)━━!!` の `)━━!!`) は 「全 char が装飾なら落とす」
  判定に引っかからず丸ごと残っていた。 前者は前後の装飾 token から顔文字パーツと
  判定して落とし (ただし 「a」 「w」 だけの発言は残す)、 後者は未変換 passthrough
  token の reading から装飾 char だけ除く (句読点は pause 情報なので残す)。

- **数字に接する空白を英字 passthrough が覆い、 数値が読まれなかった**
  (「最大震度:2 M3.7」 → 「サイダイシンド:2 M3。 ナナ」)。 英字 range は
  「Hello World」 を 1 語に保つため空白を含めていたが、 数字に接する空白まで含めて
  「2 M3」 を 1 つの passthrough 候補にしていた。 空白は **英字どうしの間だけ** range を
  繋ぐようにし (「Hello World」 は従来どおり 1 range)、 「2」 「M3」 「3.7」 を
  数値側が読めるようにした。 話し言葉 5,000 文の読み精度評価で検出。
  内部評価コーパスでは 4,144 行が変化し、 いずれも読まれずに残っていた数字が読まれる
  方向 (「pekora 2026」 → ニセンニジュウロク、 「+1 8000円」 → プラスイチ ハッセンエン)。
  既知の限界: 打率の 「.167」 は ヒャクロクジュウナナ と読む (従来は未読で素通し)。

- **カ行で始まる単位の前で促音化していなかった** (「100kg」 → ヒャクキログラム)。
  数値 + SI 単位は読みを連結するだけだったので、 6 / 8 / 10 / 100 / 百の位 +
  キロ系の単位を ロッ / ハッ / ジュッ / ヒャッ に揃えた (100kg = ヒャッキログラム、
  170km = ヒャクナナジュッキロメートル、 600kg = ロッピャッキログラム)。
  1 は イチキロ が一般的なので変えず、 カ行以外の単位 (6L = ロクリットル) にも効かない。
  内部評価コーパスでは 779 行が変化し、 全件が キロ系単位の促音化。

- **`silence_symbols` がチャットの笑いの 「w」 を残していた**。 日本語の直後に付く
  w の連なり (「草www」 「それはないだろw」 「すごいｗｗｗｗ」) は語ではないので落とす。
  英字語の続き (「LOLww」)、 w 以外を含む語 (「Wii」)、 直前が無い / 空白の w は残す
  (発言が丸ごと空になるのを避ける)。 `silence_symbols` 有効時のみで、 既定の挙動は
  変わらない。 素の VOICEVOX は笑いの w を 98% で 「ダブリュー」 と読むことを評価で確認済み。

- **`split_for_aquestalk` が 「句境界の無い長大入力」 を分割できなかった**。
  「あ」 ×3000 のような入力は 1 アクセント句のままなので句単位では切れず、
  3,002 文字の chunk がそのまま残っていた (実機なら ERROR 120 = 記号列長超過)。
  1 句だけで `max_len` を超える場合は **文字数でハード分割**する fallback を追加。
  内部評価コーパスの構造検査 (不正文字 / 核の重複 / 句数 / 長さ) で検出。

- **空白の直後の数字で助数詞 / 単位が効かなかった** (「優勝 5回」 → 「ゆうしょう 5かい」)。
  英字 range は 「Hello World」 を 1 語に保つため空白を含む仕様だったが、 その結果
  「 5」 が range になり、 passthrough (band 100) が数字を覆って数値 provider に
  届かなくなっていた。 range の前後の空白を落として解決。
  副産物として 「Bye」 「BGM」 のような **空白区切りの英単語が個別に loanwords 引き**されるようになった
  (従来は句全体が 1 range で lookup miss していた)。 内部評価コーパスで 7,607 行が変化。
...
- **数値 + 大数スケール の直後に来る 「読めない単位」 を surface のまま読みに混ぜていた**
  (内部評価コーパスの走査で発覚: 読みに漢字が残る行 1,050 件の主因)。
  `try_scale` は trailing unit が SI 単位にも counter rule にも無いとき
  `reading.push_str(u)` していたため、 「100万目標」 → 「ヒャクマン目ヒョウ」、
  「100万目指せる」 → 「ヒャクマン目サセル」 のように **漢字が読みに残った**
  (「N万目標 / N万目前 / N万目指す」 が全滅)。 修正は **候補ごと捨てる**
  (= 「100万」 + 「目標」 の path が選ばれる)。 読みに surface を混ぜる経路は他に無い。

- **dict entry が Lindera token の途中で終わると、 その entry が黙って無視されていた**
  (内部評価コーパスの A/B diff で発覚)。 候補 edge は Lindera の token 境界からしか生えないため、
...
- **英字語内のハイフンが 「マイナス」 と読まれ、 loanwords 辞書が効かなかった**
  (実運用の誤読報告: 「ポケットWi-Fi」 → 「ポケットマイナス」)。 原因は 2 つ:
  (1) 英字 range の切り出しが `-` で止まり `Wi` / `-` / `Fi` に割れるため、 辞書にある
  `"Wi-Fi" = "ワイファイ"` に届かない。 (2) `-` は `symbols.toml` の 「マイナス」 が無条件で
  当たる。 修正は、 **英字に挟まれたハイフン類 / アポストロフィ類を英字語の一部として
  1 range に含め** (`Wi-Fi` / `Blu-ray` / `don't`。 `3-1` や `A - B` は従来通り)、 lookup は
  完全一致 → 連結記号除去 (`wi-fi` → `wifi`) の 2 段で引くようにした。 加えて **`-` の
  「マイナス」 読みを 〜 と同じく数字 context 限定** にし (`3-1` / `-3` はマイナス、
  `あ-い` / `終わり-` は無音)、 英字間や区切り用途のハイフンを読まないようにした。
  辞書に無いハイフン語 (`Blu-ray` 等) は語全体が passthrough になる (= TTS engine 側の
  英語読みに任せる。 従来は 「Bluまいなすray」)。
...
- **match condition 数を `Score::match_hits` に重み付き累積** (`scoring/matcher.rs` /
  ...
  **広め条件** (`prev_ends_any` / `next_starts_any` / `next2_starts_any` / `prev_char_type` /
  ...
  内部評価コーパス A/B では 217 行が変化し、 「今日中: いまにっちゅう → きょうちゅう」 系
...
- **`NameBoundaryPass` (人名+敬称の token 衝突補正、`scoring/names.rs`)**: 「白上さん」 が
  ...
  再分割 / merge する (普通名詞は対象外 = 誤爆防止)。 内部評価コーパス A/B で副作用 1 件
...
- **`furigana serve` の rate limit を config 化** (`config.rs` / `serve/mod.rs`):
  ...
  高頻度の短文用途や負荷試験で緩められる ([CONFIG.md](docs/CONFIG.md) 参照)。
